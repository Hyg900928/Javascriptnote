...
const jar = new CookieJar(new FileCookieStore(path.resolve(__dirname, './cookie.json')));
...
function request(axiosConfig, customOptions, loadingOptions) {
    const service = wrapper(axios.create({
        jar,
    ...
    service.interceptors.response.use(
        response => {
            ...
            const milliseconds = Math.floor((currentTime - startTime) % 1000);
            response.headers['request-duration'] = `${seconds}.${milliseconds}`;
